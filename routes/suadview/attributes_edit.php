...
if (!empty($id)) {
    $stmt = $pdo->prepare("SELECT * FROM atributos WHERE id = :id");
    $stmt->execute(['id' => $id]);
    $category = $stmt->fetch();

    $nombre = $category['atributo'];
}
?>
...
<body>
    ...
    <script>
        document.getElementById("category").addEventListener("submit", async function(event) {
            ...
            const atributo = document.getElementById("dm-ecom-product-name").value.trim();
            const id = document.getElementById("id") ? document.getElementById("id").value.trim() : null;

            if (!atributo || (action === "edit" && !id)) {
                document.getElementById("message").textContent = "El atributo es obligatorio y el ID si estás editando.";
                return;
            }
