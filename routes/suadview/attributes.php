...
<head>
    ...
    <title>Piel Canela | Atributos</title>
...
<body>
    ...
    <script>
        async function loadAttributes() {
            try {
                const response = await fetch("/src/api/attributes/read_attributes.php");
                ...
                if (result.status === "success") {
                    ...
                    result.data.forEach(attribute => {
                        ...
                        row.innerHTML = `
                    <td class="text-center fs-sm">
                        <strong>ID.${attribute.id.toString().padStart(6, "0")}</strong>
                    </td>
                    <td class="d-none d-md-table-cell fs-sm">
                        <a class="fw-semibold" href="javascript:void(0)">${attribute.atributo}</a>
                    </td>
                    <td class="text-center fs-sm">
                        <a class="btn btn-sm btn-alt-secondary edit-btn" data-id="${attribute.id}" data-name="${attribute.atributo}" href="javascript:void(0)">
                            <i class="fa fa-fw fa-eye"></i>
                        </a>
                    ${attribute.id !== 4 ?
                        `<a class="btn btn-sm btn-alt-secondary delete-btn" data-id="${attribute.id}" href="javascript:void(0)">
                            <i class="fa fa-fw fa-times text-danger"></i>
                        </a>` : ""}
                    </td>
                `;
        ...
        function attachEventListeners() {
            document.querySelectorAll(".edit-btn").forEach(button => {
                button.addEventListener("click", function() {
                    ...
                    if (confirm("¿Estás seguro de que deseas editar este atributo?")) {
                        window.location.href = `/routes/suadview/attributes_edit.php?id=${id}`;
                    }
                });
            });
            ...
            document.querySelectorAll(".delete-btn").forEach(button => {
                button.addEventListener("click", async function() {
                    ...
                    if (confirm("¿Estás seguro de que deseas eliminar este atributo?")) {
                        ...
                        loadAttributes(); // Recargar categorías después de eliminar
                    }
                });
            });
        ...
        async function deleteAttribute(id) {
            try {
                const response = await fetch("/src/api/attributes/delete_attribute.php", {
                    method: "POST",
                    headers: {
        ...
        async function showAttribute(id) {
            ...
            try {
                const response = await fetch("/src/api/attributes/all_views_attribute.php");
