        ...
        if (result.status === "success") {
            ...
            result.data.forEach((category) => {
                ...
                toggleButton.addEventListener("click", (e) => {
                    ...
                    document
                        .querySelectorAll(".category-item.open")
                        .forEach((item) => {
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    if (categoriesSlider) {
        new KeenSlider(categoriesSlider, {
            ...
            created: function (instance) {
                ...
                arrowLeft.innerHTML = "❮";
                arrowRight.innerHTML = "❯";
                categoriesSlider.appendChild(arrowLeft);
                categoriesSlider.appendChild(arrowRight);
                arrowLeft.addEventListener("click", () => instance.prev());
                arrowRight.addEventListener("click", () => instance.next());
            },
            slides: {
    ...
    if (thumbs) {
        const slider = new KeenSlider(thumbs, {
            loop: true,
            slides: {
                perView: 5,
                spacing: 10,
                ...
                getProductCartById(productIdCart)
                    ...
                    .catch(() => {
                        ...
                        if (
                            ...
                            console.error(
                                "Datos inválidos para el producto:",
                                product,
                            );
                            return;
                        }
                    ...
                    .then((data) => {
                        ...
                        if (modal) {
                            ...
                            modal.querySelector("img").src =
                                BASE_URL + data.data.imagen.split(",")[0];
                            modal
                        ...
                        document
                            .querySelectorAll(
                                ".product-modal input[type=radio]",
                            )
                            .forEach((input) => {
                                input.addEventListener("click", (e) => {
                                    const image =
                                        input.getAttribute("data-image");
                                    const productImage =
            ...
            getProductCartById(productId)
                ...
                .catch(() => {
                    ...
                    const quantity = parseFloat(
                        productElement.querySelector("input[name='cantidad']")
                            .value,
                    );
    ...
    document.querySelectorAll(".btn[data-action=buy]").forEach((button) => {
        button.addEventListener("click", (e) => {
            ...
            if (!productId) {
                console.error(
                    "No se encontró el ID del producto para comprar.",
                );
                return;
            }
            ...
            document.querySelector(".checkout-modal").dataset.checkout =
                "product";
        });
    });
        ...
        comprarCarritoBtn.addEventListener("click", () => {
            getCartItems()
                ...
                .catch((error) => {
                    console.error(error);
                    alert(
                        "Hubo un error al obtener los productos del carrito.",
                    );
                });
        });
    } else {
            ...
            getCartItems()
                .then((cartItems) => {
                    ...
                    clearCart()
                        .then(() => {
                            console.log(
                                "Carrito vaciado después de la compra.",
                            );
                            displayCartItems();
                        })
